...
def generate_G_from_H(H,weight_matrix,variable_weight = False): # 从邻接矩阵构建超图
    """
    ...
    """
    if type(H) != list:
        return _generate_G_from_H(H,weight_matrix,variable_weight)
    else:
        G = []
        for sub_H in H:
            G.append(generate_G_from_H(sub_H,weight_matrix,variable_weight))
        return G


def _generate_G_from_H(H, weight_matrix, variable_weight= False):
    """
    calculate G from hypgraph incidence matrix H
    :param H: hypergraph incidence matrix H
    :param variable_weight: whether the weight of hyperedge is variable
